...
def scalarize(value, default=0.0) -> float:
    """
    Convert scalar / 0-d array / 1-element array to a Python float safely.
    """
    try:
...
def analyze_file(path: str, sr=44100, n_fft=4096, hop_length=512, fmin=27.5, fmax=8000.0):
    ...
    try:
        f0, voiced_flag, voiced_prob = librosa.pyin(
            y_harm, fmin=fmin, fmax=fmax, sr=sr, frame_length=n_fft, hop_length=hop_length
        )
        print('pyin OK')
    except Exception:
    ...
    if not events:
        ...
        for t in ot:
            ...
            events.append({
                ...
                'pitch_hz': 0.0, 'pitch_midi': 0.0,
                'amp': 0.0, 'centroid': 0.0, 'rolloff': 0.0, 'pan': 0.0
            })
    ...
    for ev in events:
        ...
        # small randomness for artistic texture
        poly = [
            (float(x) + (np.random.rand() - 0.5) * 0.001,
             float(y) * (0.9 + (np.random.rand() - 0.5) * 0.2))
            for x, y in poly
        ]
    ...
    try:
        ...
        tempo = scalarize(tempo, default=0.0)
...
def render_detailed_shapes(analysis: dict, out_base: str, size_px=2048, dpi=300):
    ...
    for ev in events:
        ...
        for (t_rel, lvl) in adsr:
            ...
            gy = y0 + (lvl - 0.5) * y_scale
            poly_pts.append((gx, gy))

        # close polygon with small base
...
def save_detailed_csv(events: list, out_csv: str):
    keys = [
        'start', 'end', 'dur', 'pitch_hz', 'pitch_midi', 'amp', 'centroid', 'rolloff', 'pan',
