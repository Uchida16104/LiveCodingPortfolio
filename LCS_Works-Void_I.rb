  ...
  if n.nil?
    ...
    n.each { |nn|
      if nn.respond_to?(:round)
        midi_note_on(nn, opts)
        $notes_to_off << [nn, opts]
      end
    }
...
define :chtree do |note, mode, time|
  ...
  if mode == "maj"
    ...
    minch1.times do
      play_midi (chord note, minch2).reverse.shuffle.tick
      play_midi (chord note, minch2).shuffle.tick if one_in(2)
      play_midi (chord note, minch2).tick if one_in(3)
      play_midi (chord note, minch2).choose if one_in(4)
      play_midi (chord note, minch2).reverse.choose if one_in(5)
      play_midi (chord note, minch2).shuffle.choose if one_in(6)
      play_midi (chord note, minch2).reverse.shuffle.choose if one_in(7)
      play_midi (chord note, minch2) if one_in(8)
      d = time / minch1.to_f
      sleep d
      $notes_to_off.each { |n_opts| midi_note_off(n_opts[0], n_opts[1]) }
      $notes_to_off.clear
